...
fn get_game(r: zap.Request, c: *Context) bool {
    ...
    if (r.getParamSlice("list") != null) {
        var json_to_send: []const u8 = undefined;
        if (state.writeGamesJson(&buf)) |json| {
            json_to_send = json;
        } else {
            json_to_send = "null";
        }
...
fn create_game(r: zap.Request, c: *Context) bool {
    r.setContentType(.JSON) catch return false;
    if (c.session == null) {
        r.sendJson("{\"error\":\"You must be logged in\"}") catch return false;
        return true;
    ...
    if (r.getParamStr(SharedAllocator.getAllocator(), "name", false)) |val| {
        if (val) |*str| {
            defer str.deinit();
            var game = Game.init(str.str, c.session.?.user.name);
            if (r.getParamStr(SharedAllocator.getAllocator(), "mode", false)) |mode_val| {
                if (mode_val) |*mode| {
                    defer mode.deinit();
                    if (std.meta.stringToEnum(Game.Mode, mode.str)) |parsed_mode| {
                        game.mode = parsed_mode;
                    }
                }
            } else |_| {
                // it's fine, fall through
            }
            state.addGame(&game) catch return false;
            r.sendJson("{\"result\":\"added\"}") catch return false;
            return true;
        } else {
            r.sendJson("{\"error\":\"missing `name` parameter\"}") catch return false;
            return true;
...
fn start_game(r: zap.Request, c: *Context) bool {
    r.setContentType(.JSON) catch return false;
    if (c.session == null) {
        r.sendJson("{\"error\":\"You must be logged in\"}") catch return false;
        return true;
    ...
    if (r.getParamSlice("id")) |str_id| {
        const id = std.fmt.parseInt(u64, str_id, 10) catch 0;
        if (state.games.getPtr(id)) |game| {
            if (!std.mem.eql(u8, &c.session.?.user.name, &game.creator)) {
